...
prev: {current.prev.node_id if current.prev is not None else "not found"}
next: {current.next.node_id if current.next is not None else "not found"}"""
	...
	def remove_from_list(self, node_id: int) -> None:
		...
		# otherwise, traverse the list to find the certain ID
		current = self.root

		while current is not None:
			if current.node_id == node_id:

				#	update the neighbur's links
				if current.prev is not None:
					current.prev.next = current.next
				#end if

				#	if this is not the last node, update the next node as well
